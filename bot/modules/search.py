...
def torserbut(update, context):
    ...
    if user_id != int(data[1]):
        ...
        if tool == 'api':
            ...
            editMessage(f"<b>Searching Your File <i>{key}</i>\nTorrent Site:- <i>{site.capitalize()}</i></b>", message)
    ...
    if tool == 'api':
        ...
        try:
            ...
            if isinstance(search_results, list):
                ...
                return editMessage(f"কোনো ফাইল পাওয়া যায়নি আপনি ফাইলটির আসল নাম শুদ্ধভাবে জেনে পরবর্তীতে আবার খুঁজুন,ধন্যবাদ। No Result Found For <i>{key}</i>\nTorrent Site:- <i>{SITES.get(site)}</i>", message)
...
def _getResult(search_results, key, message, tool):
    ...
    path = [telegraph.create_page(
                title='Iha-Nasim-bot Torrent Search',
                content=content
