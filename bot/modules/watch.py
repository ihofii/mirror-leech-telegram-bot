...
def _watch(bot, update, isZip=False, isLeech=False):
    ...
    if not is_url(link):
        help_msg = "<b>প্রথমে watch কমান্ড দিয়ে পরে লিঙ্ক দিন, এবং পরে নিজের চাহিদা অনুযায়ী সিলেক্ট করুন।:</b>"
        help_msg += "\nচাইলে ভিডিওর নাম নিজের ইচ্ছেমতো দিতে পারবেন। কাস্টম ভিডিওর নাম দেওয়ার জন্যে - প্রথমে watchx কমান্ড দিয়ে পরে লিঙ্ক দিন, তারপরে যে কোয়ালিটিতে ভিডিও ডাউনলোড করতে চান সেই নাম্বার দিন, শেষে | চিহ্ন দিয়ে কাস্টম নাম দিন।"
        help_msg += "\n\n<b>যেমন ---</b>"
        help_msg += "\n/watchx https://youtube/facebook/etc/abcdef 720 |IHNASIM.mp4/mp3\n"
        msg += "এভাবে কমান্ড দিলে ভিডিও ৭২০পি তে ডাউনলোড হবে এবং নাম হবে <b>IHNASIM.mp4</b>, আপনি চাইলে ভিডিওটি অডিওতেও ডাউনলোড করতে পারবেন।"
...
def select_format(update, context):
    ...
    uid = task_info[1]
    if user_id != uid:
        return query.answer(text="Sorry, Something is wrong.", show_alert=True)
    elif data[2] == "dict":
        query.answer()
        qual = data[3]
